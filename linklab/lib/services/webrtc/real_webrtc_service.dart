...
class RealWebRTCService {
  ...
  /// 发送ICE候选的回调
  Function(String candidate, String? sdpMid, int? sdpMLineIndex)? onIceCandidate;
  ...
  Future<bool> checkPermissions() async {
    try {
      ...
      _emitEvent(WebRTCEventType.error, error: '权限检查失败: $e');
      return false;
    }
  }
  ...
  Future<CallInfo> initializeCallAsSeeker({
    ...
    try {
      ...
      _emitEvent(WebRTCEventType.error, error: '初始化通话失败: $e');
      await _cleanup();
      rethrow;
    } finally {
  ...
  Future<void> _createPeerConnection() async {
    try {
      ...
      _emitEvent(WebRTCEventType.error, error: '创建PeerConnection失败: $e');
      throw Exception('创建PeerConnection失败: $e');
    }
  }
  ...
  Future<void> createOffer() async {
    ...
    try {
      ...
      _emitEvent(WebRTCEventType.error, error: '创建Offer失败: $e');
  ...
  Future<void> handleOffer(String sdp, String type) async {
    ...
    try {
      ...
      _emitEvent(WebRTCEventType.error, error: '处理Offer失败: $e');
      throw Exception('处理Offer失败: $e');
    }
  }
  ...
  Future<void> handleAnswer(String sdp, String type) async {
    ...
    try {
      ...
      _emitEvent(WebRTCEventType.error, error: '处理Answer失败: $e');
      throw Exception('处理Answer失败: $e');
    }
  }
  ...
  Future<void> _attemptReconnect() async {
    ...
    _reconnectAttempts++;
    print('[WebRTC] 尝试重连 ($_reconnectAttempts/${WebRTCConfig.maxReconnectAttempts})');

    await Future.delayed(Duration(milliseconds: WebRTCConfig.reconnectInterval));
  ...
  Future<void> startCall(String roomId) async {
    ...
    print('[WebRTC] 通话开始: roomId=$roomId');
  }
  ...
  Future<void> endCall(CallEndReason reason) async {
    print('[WebRTC] 结束通话: reason=$reason');
  ...
  Future<void> _collectStats() async {
    ...
    try {
      ...
      final packetLossRate = totalPackets > 0 ? packetsLost / totalPackets : 0.0;
  ...
  void _emitEvent(WebRTCEventType type, {dynamic data, String? error}) {
    if (!_isDisposed) {
      _eventController.add(WebRTCEvent(
        type: type,
        data: data,
        error: error,
      ));
