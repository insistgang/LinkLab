...
class SOSService {
  ...
  Future<void> _broadcastSOS(String sosId, Position position) async {
    try {
      await http.post(
        Uri.parse('${_supabase.supabaseUrl}/functions/v1/push-notifier'),
        headers: {
  ...
  void _startEscalationTimer(String sosId, Position position) {
    _escalationTimer = Timer.periodic(const Duration(minutes: 5), (timer) async {
      ...
      await _supabase.from('sos_requests').update({
        'escalation_level': _escalationLevel,
      }).eq('id', sosId);
    });
  }
        ...
        .onPostgresChanges(
          ...
          callback: (payload) async {
            ...
            if (status == 'responded') {
              ...
              final volunteerId = newRecord['responder_id'];
              if (volunteerId != null) {
  ...
  Future<void> _connectToVolunteer(String sosId, String volunteerId) async {
    ...
    // 创建房间ID
    final roomId = 'sos_${sosId.substring(0, 8)}';
  ...
  Future<void> cancelSOS() async {
    ...
    _currentSOSId = null;
    _escalationLevel = 0;
    _updateSOSState(SOSState.cancelled);
  }
  ...
  Future<void> resolveSOS() async {
    ...
    _currentSOSId = null;
    _escalationLevel = 0;
    _updateSOSState(SOSState.resolved);
  }
