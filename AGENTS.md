...
> 適用範圍：`E:\vscode_project\LinkLab` 整個 workspace
...
1. **產品範圍對齊**：企劃書願景先映射到 `F1/F9/F11/F13/F33/F36`，其餘內容默認進入 V1.0、靜態藍圖或歸檔。
2. **主鏈路對齊**：Flutter 默認啓動、首頁、AI、匹配、通話、SOS、個人偏好必須只服務 3 分鐘 Demo 主線，不讓真實 Supabase、真實 WebRTC、社羣、積分、後臺等非 MVP 模塊污染入口。
3. **驗收證據對齊**：每項完成都必須能落到代碼入口、可見 UI 狀態、Demo fallback、無障礙要求和 `flutter analyze` / `flutter test` / 手動 3 分鐘腳本。
...
| `ai_processing` | 緊急意圖 | F13 SOS 內部流程 | 顯示 10 秒撤銷窗口和後續通知狀態；不新增 `help_request` 主狀態 |
...
- SOS 在競賽 Demo 中由 `F13` 頁面內部 phase 承接，可通過 `intent` / `urgency` / `metadata` 記錄緊急語義，但不得把 `sos_triggered` 加回 Flutter 主狀態機。
...
說明：`emergency` / `SOS` 是意圖與流程類型，不是當前 Flutter Demo 的 `help_request` 主狀態；競賽版以 F13 內部 UI phase 展示撤銷、廣播、聯繫人通知和完成態。
